...
(function($) {
	...
	$.fn.timepicki = function(options) {
		...
		return this.each(function() {
			...
			if(settings.remove){
				new_ele.append(
					"<div><a href='javascript:void(0)' class='reset_time'>REMOVE</a></div>");
			}
			if(settings.closable){
				new_ele.append(
					"<div><a href='javascript:void(0)' class='close_time'>DONE</a></div>");
			}
			...
			$('.reset_time').on("click", function(event) {
				console.log('reset');
				$(this).closest('.time_pick').find('.time-picki-picker').val("");
				close_timepicki();
			});	
